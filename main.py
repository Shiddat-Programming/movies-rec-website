...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 'limits' aur 'retries' add karna behtar hai
    async with httpx.AsyncClient(
        ...
        try:
            ...
            # Agar 429 error (Too Many Requests) aaye toh wait karein
            if r.status_code == 429:
                return {"results": [], "error": "Rate limit exceeded"}

            if r.status_code != 200:
                # Terminal mein print karein taaki aap dekh sakein kyun fail hua
                print(f"TMDB API Error: {r.status_code} - {r.text}")
                raise HTTPException(status_code=502, detail="TMDB Error")

            return r.json()
            
        except httpx.ReadTimeout:
            print("Timeout error: TMDB took too long to respond.")
